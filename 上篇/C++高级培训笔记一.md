...
inline关键字用于定义内联函数（inline function）。**内联函数是在编译期间将函数调用直接替换为函数体**，从而避免函数调用的开销。
替代宏定义：

在C语言中，经常使用宏定义（#define）来定义小的、频繁调用的代码片段。宏定义的主要缺点是它们只是简单的文本替换，容易引入错误和调试困难。
C++引入inline关键字作为宏定义的替代品，目的是提供一种更安全、更清晰的方式来定义小函数。
减少栈空间消耗：

**频繁调用的小函数在传统函数调用中会导致大量的栈空间（栈内存）消耗**，因为每次调用函数都需要在栈上保存返回地址、参数和局部变量。
使用内联函数可以减少这种消耗，因为内联函数的调用不会发生实际的函数调用，而是直接将函数体插入到调用点，从而节省了栈空间。
